...
chart(DATA, Y_VAR ~ X_VAR %col=% GROUP_VAR %shape=% GROUP_VAR) +
...
chart(DATA, VALUE_VAR ~ DATE_VAR %col=% GROUP_VAR %linetype=% GROUP_VAR) +
...
chart(DATA, MEAN_VAR ~ CATEGORY_VAR) +
...
```r
# Side-by-side bars
chart(DATA, ~ CATEGORY_VAR %fill=% GROUP_VAR) +
...
chart(DATA, ~ CONTINUOUS_VAR %fill=% GROUP_VAR) +
...
chart(DATA, CONTINUOUS_VAR ~ CATEGORY_VAR) +
...
chart(DATA, CONTINUOUS_VAR ~ CATEGORY_VAR %fill=% CATEGORY_VAR) +
...
chart(DATA, Y_VAR ~ X_VAR | FACET_VAR) +
...
## Facet Wrap Template when arguments must be added to the facets
...
chart(DATA, X_VAR ~ Y_VAR %fill=% VALUE_VAR) +
...
chart(DATA, Y_VAR ~ X_VAR) +
  ...
  # Text annotation
  annotate("text", x = X_POSITION, y = Y_POSITION, label = "ANNOTATION_TEXT",
