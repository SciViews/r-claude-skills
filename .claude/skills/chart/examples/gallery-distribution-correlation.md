...
Advanced ggplot2/chart techniques from the R Graph Gallery for distribution and correlation visualizations. These patterns go beyond basic usage and show production-ready techniques.
...
```r
chart(data, value ~ group %fill=% group) +
...
data |>
  ...
  chart(value ~ category %fill=% category) +
...
chart(data, value ~ main_group %fill=% subgroup) +
  geom_violin(position = position_dodge(width = 0.9)) +
  scale_fill_brewer(palette = "Set2")

# Faceting approach (clearer for many subgroups)
chart(data, value ~ subgroup %fill=% subgroup) +
  geom_violin() +
...
```r
chart(data, ~ value %fill=% category) +
...
```r
chart(data, ~ x %col=% group %label=% group) +
  geomtextpath::geom_textdensity(size = 4, fontface = 2, hjust = 0.2, vjust = 0.3) +
...
```r
chart(data, value ~ category %fill=% category) +
...
```r
chart(data, category ~ value %fill=% category) +
...
```r
chart(data, category ~ value %fill=% category) +
  ggridges::geom_density_ridges(stat = "binline", bins = 30, scale = 0.95, alpha = 0.7) +
  scale_fill_viridis_d()
```
...
```r
chart(data, var2 ~ var1 %col=% category %size=% var3 %shape=% group) +
...
```r
chart(data, var2 ~ var1 %label=% name) +
...
```r
p <- chart(data, var2 ~ var1 %col=% group) +
  geom_point()
...
- Apply ggExtra::ggMarginal() to saved plot
...
chart(cor_melted, Var2 ~ Var1 %fill=% value) +
...
ggpairs(data, columns = c("var1", "var2", "var3", "var4"),
  aes(color = category, alpha = 0.5), upper = list(continuous = "cor"),
...
```r
chart(data, life_exp ~ gdp %size=% population %col=% continent) +
  geom_point(alpha = 0.6) +
...
data |>
  ...
  chart(var2 ~ var1 %size=% size_var %col=% category) +
...
```r
chart(large_data, var2 ~ var1) +
  hexbin::geom_hex(bins = 50) +
...
data |>
  ...
  chart(category ~ NULL %col=% change_direction)) +
...
chart(data, value ~ time %col=% group) +
  ...
  geom_text(data = last_points, aes(label = group),
