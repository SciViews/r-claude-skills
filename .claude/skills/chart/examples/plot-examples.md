...
chart(iris, Sepal.Width ~ Sepal.Length %col=% Species %shape=% Species) +
...
chart(mtcars, mpg ~ wt %size=% hp %col=% factor(cyl)) +
...
chart(economics_recent, unemploy ~ date) +
...
economics_long <- economics |>
  ...
  pivot_longer(cols = c(psavert, uempmed), names_to = "variable",
    values_to = "value")

# Plot with distinct colors and shapes
chart(economics_long, value ~ date %col=% variable %linetype=% variable) +
...
chart(summary_data, mean_mpg ~ factor(cyl)) +
...
```r
# Side-by-side bars
chart(mpg, ~ class %fill=% factor(year)) +
...
chart(class_counts, reorder(class, n) ~ n) +
...
chart(iris, ~ Sepal.Length %fill=% Species) +
...
chart(iris, ~ Sepal.Length %col=% Species) +
...
```r
# Distribution shape
chart(iris, Sepal.Length ~ Species %fill=% Species) +
...
chart(economics_long, value ~ date) +
  geom_line(colour = "steelblue", linewidth = 1) +
...
chart(cor_data, Var2 ~ Var1 %fill=% value) +
...
chart(economics, unemploy ~ date) +
  ...
  geom_line(colour = "steelblue", linewidth = 1) +
  # Annotation
  annotate("text", x = as.Date("2008-06-01"), y = 15000,
...
# Create plot
p <- chart(plot_data, Sepal.Width ~ Sepal.Length %col=% Species %shape=% Species) +
  # Data layers
  geom_point(size = 3, alpha = 0.7) +
  ...
  scale_colour_manual(values = c("versicolor" = "#0072B2", "virginica" = "#D55E00"),
    labels = c("I. versicolor", "I. virginica")) +
  scale_shape_manual(values = c("versicolor" = 16, "virginica" = 17),
    labels = c("I. versicolor", "I. virginica")) +
...
p1 <- chart(iris, Sepal.Width ~ Sepal.Length %col=% Species) +
  geom_point(size = 2) +
  scale_colour_viridis_d(option = "plasma", end = 0.9) +
  labs(title = "A) Sepal dimensions", x = "Length (cm)", y = "Width (cm)") +
  theme(legend.position = "none", plot.title = element_text(face = "bold"))

p2 <- chart(iris, Petal.Width ~ Petal.Length %col=% Species) +
  geom_point(size = 2) +
  scale_colour_viridis_d(option = "plasma", end = 0.9) +
  labs(title = "B) Petal dimensions", x = "Length (cm)", y = "Width (cm)") +
  theme(legend.position = "none", plot.title = element_text(face = "bold"))

p3 <- chart(iris, Sepal.Length ~ Species %fill=% Species) +
  geom_violin(alpha = 0.5) +
  geom_boxplot(width = 0.1, fill = "white", outlier.shape = NA) +
  scale_fill_viridis_d(option = "plasma", end = 0.9) +
