# Best R Graph Gallery Examples: Advanced ggplot2/chart Patterns
...
These are ggplot2, but use the rules to convert into chart (`ggplot()` -> `chart()` and `aes()` -> formula).
...
**Technique:**
```r
patchwork::inset_element(secondary_plot, left = 0.6, right = 1.0,
  top = 0.99, bottom = 0.7, clip = FALSE)
```
...
**Technique:**
```r
theme_sciviews() + # or theme_minimal() or theme_void()
theme(
