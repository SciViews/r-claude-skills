# Advanced chart/ggplot2 Techniques from R Graph Gallery
...
**Code Pattern:**
```r
chart(data, value ~ time %col=% group) +
  geom_line() +
...
chart(data, value ~ year %col=% group) +
  geom_line() +
...
chart(data, value ~ year %fill=% category) +
  ggstream::geom_stream() +  # or geom_area(position = "stack")
  annotate("text", x = 2021, y = calculated_y_position, label = "Category Name",
    hjust = 0, color = "white") +
...
**Code Pattern:**
```r
chart(data, y ~ x %fill=% category) +
  ggpattern::geom_area_pattern(aes(pattern = category),
    pattern_fill = "grey80", pattern_density = 0.5,
    pattern_spacing = 0.05, alpha = 0.7)
```
...
**Code Pattern:**
```r
chart(data, value ~ time | category) +
...
**Code Pattern:**
```r
chart(data, value ~ year %color=% category) +
  geom_line() +
  geofacet::facet_geo(~region, grid = "us_state_grid1") +  # or gb_london_boroughs_grid
...
**Code Pattern:**
```r
chart(data, value1 ~ time) +
  ggh4x::stat_difference(aes(ymin = value1, ymax = value2), alpha = 0.3) +
...
**Code Pattern:**
```r
chart(data, value ~ time %fill=% category) +
  ggstream::geom_stream(type = "proportional",  # or "mirror", "ridge"
    bw = 0.75) +
...
chart(data, value ~ fct_reorder(category, value)) +
  geom_col() +
  coord_flip()

# Method 2: Manual order
chart(data, value ~ fct_relevel(category, "First", "Second", "Third")) +
  geom_col()

# Method 3: Order by frequency
chart(data, ~ fct_infreq(category)) +
  geom_bar()

# Method 4: Reverse order
chart(data, value ~ fct_rev(fct_reorder(category, value))) +
  geom_col()
```
...
**Special Considerations:**
- chart uses factor level order, NOT data frame row order
...
chart(data, value ~ category %fill=% subcategory) +
...
```r
# Error bars
chart(data, mean ~ category) +
  geom_col(fill = "steelblue") +
  geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd),
    width = 0.2, position = position_dodge(0.9))

# Sample size annotations
chart(data, value ~ category) +
  geom_col() +
...
chart(data, value ~ category) +
  geom_col() +
  geom_text(aes(label = scales::comma(value)), vjust = 1.5, color = "white",
    fontface = "bold")
```
...
chart(data, value ~ category) +
  geom_segment(aes(x = category, xend = category, y = baseline, yend = value),
    color = ifelse(data$value > data$baseline, "darkgreen", "darkred")) +
...
```r
# Basic dumbbell
chart(data, aes(y = fct_reorder(category, gap))) +
...
chart(data, value ~ category %fill=% group) +
  geom_col() +
...
chart(data_with_gaps, value ~ as.factor(id) %fill=% group) +
  geom_col() +
  coord_polar() +
...
chart(data_with_gaps, value ~ as.factor(id)) +
  geom_col() +
  geom_text(data = label_data,
    aes(x = id, y = value + 10, label = category, hjust = hjust, angle = angle),
    size = 3) +
  coord_polar()
```
...
**Packages:** Base chart + ggplot2, custom fonts via `showtext` or `ragg`
...
chart(data, y ~ x) +
  ...
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = Inf, ymax = Inf - 0.5,
    fill = accent_red) +
  economist_theme
```
...
```r
# The Economist
chart(data, y ~ x) + geom_point() + gthemes::theme_economist()

# FiveThirtyEight
chart(data, y ~ x) + geom_point() + gthemes::theme_fivethirtyeight()

# Tufte (minimal)
chart(data, y ~ x) + geom_point() + gthemes::theme_tufte()

# Excel (ironically)
chart(data, y ~ x) + geom_point() + gthemes::theme_excel()

# Wall Street Journal
chart(data, y ~ x) + geom_point() + gthemes::theme_wsj()
```
...
**Code Pattern:**
```r
p <- chart(data, y ~ x %col=% group) +
  geom_point()

plotly::ggplotly(p)  # Instant interactivity with hover, zoom, pan
```
...
**Code Pattern:**
```r
p <- chart(data, value ~ category) +
  ggiraph::geom_col_interactive(aes(tooltip = paste0(category, ": ", value),
    data_id = category), hover_css = "fill:orange;stroke:black;cursor:pointer;")
...
library(forcats)
chart(data, value ~ fct_reorder(name, value))
...
data |>
  ...
  chart(data, value ~ name)

# Method 3: Base R
chart(data, value ~ reorder(name, value))
```
