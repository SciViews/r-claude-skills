...
Comprehensive documentation of all major chart/ggplot2 geometric objects (geoms) with aesthetics, parameters, and use cases.
...
- `col` or `colour` - Point border/line color
...
**Shapes**:
- 0-20: Single color (use `col` or `colour`)
- 21-25: Two colors (use `col` or `colour` for border, `fill` for interior)
...
# Multiple groups
chart(data, value ~ date %col=% group) +
...
# Custom linetype
chart(data, value ~ date %linetype=% group) +
...
# Stacked by fill
chart(data, ~ category %fill=% group) +
  geom_bar()
...
**Optional Aesthetics**:
- `alpha`, `col` or `colour`, `fill`, `linetype`, `linewidth`, `weight`
...
chart(data, after_stat(density) ~ x %fill=% group) +
...
chart(data, ~ x %col=% group) +
  geom_freqpoly(binwidth = 2, linewidth = 1)

# Density scale
chart(data, after_stat(density) ~ x %col=% group) +
...
**Optional Aesthetics**:
- `alpha`, `col` or `colour`, `fill`, `group`, `linetype`, `linewidth`, `weight`
...
**Optional Aesthetics**:
- `alpha`, `col` or `colour`, `fill`, `group`, `linetype`, `linewidth`
...
```r
# Contour lines
chart(volcano_data, y ~ x %z=% elevation) +
  geom_contour()
...
```r
# Draw custom shapes
chart(polygon_data, y ~ x %group=% id %fill=% group) +
  geom_polygon()
```
