...
**Solution**: Use `ggrepel::geom_text_repel()` with direction constraints.
...
chart(data, value ~ year %col=% name) +
  geom_line() +
...
**Problem**: Need precise control beyond chart/ggplot2's capabilities.

**Solution**: Combine chart/ggplot2 with `grid` package primitives.

```r
library(grid)

p <- chart(data, y ~ x) + geom_point()
print(p)
...
chart(data, y ~ x %col=% var1 %size=% var2 %shape=% var3) +
  geom_point() +
...
```r
chart(data, y ~ x %col=% category %shape=% type) +
  geom_point() +
  labs(
...
# Continuous palettes
chart(data, y ~ x %col=% value) +
  geom_point() +
  paletteer::scale_color_paletteer_c("viridis::plasma")

# Preview palettes
paletteer::paletteer_d("nord::aurora")
paletteer::paletteer_c("ggthemes::Temperature Diverging", n = 20)
```
...
paletteer::palettes_d_names  # Discrete
paletteer::palettes_c_names  # Continuous

# Extract hex codes
paletteer::paletteer_d("nord::aurora", n = 5)
```
...
chart(data_plot, y ~ x %col=% color_var) +
  geom_point() +
  scale_color_manual(
    values = c(
      "A" = "#E31A1C",
...
chart(data, value ~ reorder(category, value)) +
...
chart(data, y ~ x) +
  geom_point() +
  shadowtext::geom_shadowtext(aes(label = label), color = "white",
...
chart(data_plot, reorder(category, value) ~ value) +
  geom_segment(
    aes(xend = 0, yend = category, color = point_color,
      size = point_size * 0.3, alpha = point_alpha),
    show.legend = FALSE
  ) +
  geom_point(
