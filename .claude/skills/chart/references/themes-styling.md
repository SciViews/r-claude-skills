...
Complete guide to chart/ggplot2 themes, color scales, and visual styling for publication-ready plots.
...
#### theme_sciviews() (Default)

Near-publication-ready theme used by default by chart() (no need to specify it).
...
**When to use**: Choice to get same plots as default ggplot2 theme.
...
chart(data, y ~ x %fill=% income_bracket) +
  geom_col() +
  scale_fill_brewer(palette = "YlOrRd", type = "seq")

# Diverging for correlation heatmap
chart(corr_data, var2 ~ var1 %fill=% correlation) +
  geom_tile() +
