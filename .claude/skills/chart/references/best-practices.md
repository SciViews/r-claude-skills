# Chart Best Practices & Patterns

Expert guidance on creating effective, accessible, and publication-ready visualizations with chart.
...
**Rule**: Inside `aes()` or the formula = map variables. Outside `aes()` or the formula = set fixed values.
...
```r
# WRONG
chart(data, sqrt(sales) ~ log(income + 1)) +
  geom_point()
...
chart(plot_data, sqrt_sales ~ log_income) +
  geom_point() +
...
```r
# OVERWHELMING
chart(data, y ~ x %col=% var1 %size=% var2 %shape=% var3 %alpha=% var4) +
  geom_point()

# BETTER - focus on key relationships
chart(data, y ~ x %col=% var1 | var3) +
...
chart(data, y ~ x %col=% group %size=% value) +
  geom_point(alpha = 0.6)
```
...
chart(data, y ~ x %col=% group %shape=% group) +
  geom_point(size = 3) +
  scale_colour_brewer(palette = "Set1")

# Color + linetype
chart(data, value ~ time %col=% group %linetype=% group) +
...
```r
# Continuous: Viridis
chart(data, y ~ x %col=% temperature) +
  geom_point() +
  scale_colour_viridis_c()

# Diverging: Gradient2 with midpoint
chart(data, y ~ x %fill=% correlation) +
  geom_tile() +
...
chart(data, gdp ~ year %group=% country) +
...
scatter_plot <- function(data, x_var, y_var, color_var = NULL) {
  chart(data, aes(x = {{ x_var }}, y = {{ y_var }}, colour = {{ color_var }})) +
    geom_point(size = 3, alpha = 0.6)
}
...
chart(huge_data, value ~ category) +
...
chart(summary_data, mean_value ~ category) +
  geom_point() +
...
chart(data, y ~ x) +
  geom_point() +
  theme_sciviews(font_family = "Arial")
...
p1 <- chart(data, y ~ x) + geom_point() + labs(tag = "A")
p2 <- chart(data, z ~ x) + geom_point() + labs(tag = "B")
p3 <- chart(data, z ~ y) + geom_point() + labs(tag = "C")
